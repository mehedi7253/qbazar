    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-header d-flex align-items-center">
                <h4 class="header-title">{{ _lang('Delivery Boy List') }}</h4>
                @if(has_permission('delivery_boys.create'))
                <a class="btn btn-primary btn-sm ml-auto ajax-modal" data-title="{{ _lang('Add Delivery Boy') }}"
                    href="{{ route('delivery_boys.create') }}"><i class="icofont-plus-circle"></i> {{ _lang('Add New') }}</a>
                @endif
            </div>
                        ...
                        <tr data-id="row_{{ $user->id }}">
                            ...
                            <td class="text-center">
                                <span class="dropdown">
                                    ...
                                    <form action="{{ action('DeliveryBoyController@destroy', $user['id']) }}" method="post">
                                        {{ csrf_field() }}
                                        <input name="_method" type="hidden" value="DELETE">

                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <a href="{{ action('DeliveryBoyController@edit', $user['id']) }}"
                                                data-title="{{ _lang('Add Delivery Boy') }}"
                                                class="dropdown-item ajax-modal"><i class="icofont-edit-alt"></i>
                                                {{ _lang('Edit') }}</a>
                                            <a href="{{ action('DeliveryBoyController@show', $user['id']) }}"
