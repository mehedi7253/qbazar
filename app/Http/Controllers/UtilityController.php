...
class UtilityController extends Controller {
	...
	public function website_settings($store = '', Request $request) {
		if ($store == '') {
			return view('backend.administration.general_settings.website_settings');
		} else {
			foreach ($_POST as $key => $value) {
			...
			if (!$request->ajax()) {
				return redirect()->route('settings.website_settings')->with('success', _lang('Saved Sucessfully'));
			} else {
				return response()->json(['result' => 'success', 'action' => 'update', 'message' => _lang('Saved Sucessfully')]);
			}
		}
	...
	public function upload_logo(Request $request) {
		...
		if ($request->hasFile('logo')) {
			...
			$name = 'app-luncher.' . $image->getClientOriginalExtension();
			$destinationPath = public_path('/theme/img');
	...
	public function upload_website_logo(Request $request) {
		$this->validate($request, [
			'website_logo' => 'required|image|mimes:png|max:1024',
		]);

		if ($request->hasFile('website_logo')) {
			$image = $request->file('website_logo');
			$name = 'website-logo.' . $image->getClientOriginalExtension();
			$destinationPath = public_path('/theme/img');
			$image->move($destinationPath, $name);
			...
			if (Setting::where('name', "website_logo")->exists()) {
				Setting::where('name', '=', "website_logo")->update($data);
			} else {
				$data['name'] = "website_logo";
				$data['created_at'] = Carbon::now();
				Setting::insert($data);
			}

			\Cache::put("website_logo", $name);

			if (!$request->ajax()) {
				return redirect()->route('settings.update_settings')->with('success', _lang('Saved Sucessfully'));
			} else {
				return response()->json(['result' => 'success', 'action' => 'update', 'message' => _lang('Website Logo Upload successfully')]);
			}
