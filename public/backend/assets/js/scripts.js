(function($) {
        ...
        .on("select2:open", () => {
          ...
          $(".select2-results:not(:has(a))").append(
            '<p class="border-top m-0 p-2"><a class="ajax-modal-2" href="' +
              ...
              '" data-title="' +
              $(this).data("title") +
              '" data-reload="false"><i class="uil uil-plus-circle"></i> ' +
              $lang_add_new +
              "</a></p>"
          );
        });
  ...
  //Ajax One time Submit
  $(document).on("submit", ".assign-delivery-boy", function() {
      ...
      success: function(data) {
        ...
        if (json["result"] == "success") {
          if (json["action"] == "update") {
            ...
            $.post(
              _url + "/orders/trigger_assign_delivery_boy/" + json["data"]["id"]
            );
          }
    ...
    $.ajax({
      ...
      beforeSend: function() {
        ...
        $(elem)
          .find("button[type=submit]")
          .html(
            '<div class="spinner-border text-primary spinner-border" role="status"><span class="sr-only">Loading...</span></div>'
          );
      },
      success: function(data) {
        $("#preloader").fadeOut();
  ...
  $(document).on("change", "#order-update-form #payment_status", function() {
    $(this).val() == "paid"
      ? $("#order-update-form .transaction_id").removeClass("d-none")
      : $("#order-update-form .transaction_id").addClass("d-none");
  });
